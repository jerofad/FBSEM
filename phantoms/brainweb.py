...
#    indicesConnectivity = phantom == 144
    ...
    mumap = np.zeros(phantom.shape, dtype='float32')
    mu_bone_1_cm = 0.13
    ...
    # T1
    t1 = np.zeros(phantom.shape, dtype='float32')
    whiteMatterT1 = 154
    grayMatterT1 = 106
    skinT1 = 92
    skullT1 = 48
    ...
    t1[indicesCsf] = csfT2
    # T2
    t2 = np.zeros(phantom.shape, dtype='float32')
    whiteMatterT2 = 70
    grayMatterT2 = 100
    skinT2 = 70
    skullT2 = 100
    ...
    # Regrid first so lesions are applied at target resolution (avoids huge
    # 4-D allocations at raw 434x362x362 resolution inside random_lesion).
    need_lesions = pet_lesion or t1_lesion or t2_lesion
    if need_lesions:
        wm_mask_lr = regrid(indicesWhiteMatter.astype('float32'), [0.5, 0.5, 0.5], voxel_size)
        wm_mask_lr = zero_pad(wm_mask_lr, image_size) > 0.5

    # Free the large raw-resolution boolean masks before regrid
    del indicesCsf, indicesWhiteMatter, indicesGrayMatter, indicesSkin
    del indicesSkull, indicesMarrow, indicesDura, indicesBone, indicesAir
    del phantom
    ...
    # Apply lesions on the regrided (small) volume.
    # voxel_size is in mm so pass it as voxel_radious_mm for correct radii.
    vox_mm = float(voxel_size[0])
    if pet_lesion:
        lesion_pet = random_lesion(wm_mask_lr, num_lesions, lesion_size_mm, voxel_radious_mm=vox_mm)
        ...
        del lesion_pet
    if t1_lesion:
        lesion_t1 = random_lesion(wm_mask_lr, num_lesions, lesion_size_mm, voxel_radious_mm=vox_mm)
        lesion_values = np.zeros(num_lesions)
        indx = list(range(num_lesions))
        np.random.shuffle(indx)
        split = int(np.floor(num_lesions * (hot_cold_ratio)))
        ...
        del lesion_t1
    if t2_lesion:
        lesion_t2 = random_lesion(wm_mask_lr, num_lesions, lesion_size_mm, voxel_radious_mm=vox_mm)
        lesion_values = np.zeros(num_lesions)
        indx = list(range(num_lesions))
        np.random.shuffle(indx)
        split = int(np.floor(num_lesions * (hot_cold_ratio)))
